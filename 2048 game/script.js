...
function startMove(event){
    document.getElementById(`startEndInformationField`).innerHTML=`<h2>To start the game, press "a", "d", "w", "s" or the arrow keys.</h2><h2>Previous record:${localStorage.getItem(`maxResult`)}</h2>` 
    ...
    if(event.key == `w` || event.key == `ArrowUp` || event.key == `s` || event.key == `ArrowDown` ||
        ...
        if(firstGameCheck){
            ...
            let arrayWithSortedLines = conversionInputDataIntoMatrix(event, inputArray);
           
            arrayWithSortedLines = lineSort(arrayWithSortedLines);
            
            outputArray = formationResultsArray(event, arrayWithSortedLines);
            
            displayResultOnScreen(outputArray);
        }
        ...
        while(true){            
            ...
            if(counter == 48){
                ...
                currentMaxScore = outputArray[15];

                if(+currentMaxScore > +localStorage.getItem(`maxResult`)){
                    document.getElementById(`startEndInformationField`).innerHTML=`<h2>To start the game, press "a", "d", "w", "s" or the arrow keys.</h2><h2>Previous record:${currentMaxScore}</h2>` 
                    localStorage.removeItem(`maxResult`);
                    localStorage.setItem(`maxResult`, +currentMaxScore);
                }

                let answerAboutEndOfTheGame = confirm(`Your score:\n${currentMaxScore}\nDo you want continue?`);
                
                if(answerAboutEndOfTheGame == true){
                    for(let i = 0; i < 16; i++){
...
function conversionInputDataIntoMatrix(event, inputArray){
    let firstLine = [];
    let secondLine = [];
    let thirdLine = [];
    let fourthLine = [];
    ...
    return arrayWithSortedLines;
}

function lineSort(arrayWithSortedLines){
    for(let i = 0; i < 4; i++){
        let counter=0;
...
function formationResultsArray(event, arrayWithSortedLines){
    if(event.key == `w` || event.key == `ArrowUp`){
...
function displayResultOnScreen(outputArray){
    for(let i = 0; i < 16; i++){
        document.getElementsByClassName(`input${i}`)[0].innerHTML = ` `;
        document.getElementsByClassName(`input${i}`)[0].className = `input${i}`;
